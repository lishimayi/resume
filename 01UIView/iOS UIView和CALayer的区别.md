...
* CALayer不可以响应事件（CALayer继承自NSObject）

## UIView与CALayer的关系

* UIView 遵循 **`CALayerDelegate`** 协议，并作为其内部 `CALayer` 的 delegate。
* UIView 主要负责事件处理，CALayer 主要负责绘制。
...
* 创建UIView对象时，UIView内部会自动创建一个层（CALayer对象），通过UIView的layer属性可以访问这个层。当UIView需要显示到屏幕上时，会调用 `drawRect:` 方法进行绘制渲染，并且会将所有内容绘制在自己的层上。绘制完毕后，图层树经 Commit Transaction 提交给 Render Server，由 GPU 合成后输出到屏幕，完成 UIView 的显示。
