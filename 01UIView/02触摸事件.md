...
- iOS 中的 UIEvent 事件类型（UIEventType）：
  - 触摸事件（UIEventTypeTouches）- 本文探讨
  - 运动事件（UIEventTypeMotion）- 包括加速计、陀螺仪等
  - 远程控制事件（UIEventTypeRemoteControl）- 如耳机线控
  - 按压事件（UIEventTypePresses）- iOS 9.0+，用于物理按钮（如 Apple TV 遥控器、游戏手柄按钮等）
  - 滚动事件（UIEventTypeScroll）- iOS 13.4+，用于鼠标/触控板滚轮
  - 悬停事件（UIEventTypeHover）- iOS 13.4+，用于指针悬停
  - 变换事件（UIEventTypeTransform）- iOS 13.4+，用于旋转等变换操作
...
> 下文生命周期、Hit-Testing、响应者链均围绕 **触摸事件（UIEventTypeTouches）** 展开；其他 `UIEventType` 的分发入口不同，不走同一套 hit-testing 流程。

### 系统响应阶段：划分以下步骤

1. 手指触碰屏幕，屏幕将触摸事件交由 IOKit.framework 处理。
2. IOKit.framework 将触摸事件封装成一个 IOHIDEvent 对象，并通过 mach port 传递给 SpringBoard 进程。
   - mach port : 进程端口，用于进程间通信。
   - SpringBoard.app : 一个系统进程，可理解为桌面系统。可以统一管理和分发系统接收到的触摸事件
...
- APP 进程的 mach port 接收到 SpringBoard 进程传递来的触摸事件，主线程的 runloop 被唤醒，触发了 source1 回调。
...
- 手指触摸一次屏幕对应一个 UITouch 对象，多个手指同时触摸对应生成多个 UITouch 对象
...
- 触摸的目的是生成触摸事件供响应者响应，一个触摸事件对应一个 UIEvent 对象，其中的 `type` 属性标识了事件的类型（即前文列出的多种 `UIEventType`）。
- 对于触摸事件（`UIEventTypeTouches`），UIEvent 中包含触发该事件的触摸对象集合；一个触摸事件可能由多个手指同时触摸产生，可通过 `allTouches` 获取。按压等其他类型事件则对应各自的对象集合（如 presses），不走 `allTouches`。
...
- 所有派生自 UIResponder 的对象，本身都具备响应事件的能力：
  - 以下类的实例都是响应者：
    ...
    - UIViewController
    - UIApplication
    - AppDelegate（通常继承自 UIResponder）
- 针对不同 `UIEventType`，UIResponder 提供了对应的处理方法：触摸用 `touches*`，运动用 `motion*`，按压用 `presses*`，远程控制用 `remoteControlReceivedWithEvent:` 等。本文只展开触摸相关的 4 个方法：
...
- 一次点击贯穿了诸多可以响应这次点击的响应者，需要通过优先级排序找到最佳响应者，这个过程叫做 Hit-Testing。命中的视图称为 hit-tested view，它是后续响应者链传递的起点。
- 注意：hit-tested view ≠ first responder。first responder 是另一套概念（例如成为键盘输入目标的那个响应者），不要和 Hit-Testing 的命中视图混为一谈。
...
|--|响应者链|
|---------|------|
| 响应者|UIView（hit-tested view，最佳响应者）|
|nextResponder 指针|↓|
|响应者|superView|
|nextResponder 指针|↓|
|···|···|
